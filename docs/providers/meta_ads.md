...
Official API path: (1) identity + location confirmation at facebook.com/ID with government ID (hours to days); (2) Meta developer account + app with the "Ad Library API" product, accepting the "Authorized Access to Public Information" terms; (3) user access token with `ads_read` (1h short-lived, exchangeable for 60-day). No formal App Review is reported for `ads_archive`. No company required — individuals verify with personal ID. **Egypt: likely possible** — Egypt is on Meta's ad-transparency enforcement expansion list (2019 announcement + 2022 enforcement coverage), but the current supported-country list at facebook.com/ID is login-gated and could not be confirmed; treat as high-probability, confirmed only by attempting it. Web UI: no account needed.

## 3. Geographic & dataset coverage — **the crux of this record**

**The API is NOT a general commercial-ads API:**

- **Outside the EU/UK**, `ads_archive` returns only **social-issue/electoral/political ads** (plus US special-category ads). Ordinary ecommerce ads shown in the US, Egypt, GCC etc. are visible in the browser but **not returned by the API**, even with `ad_type=ALL`.
...
Implication for us: programmatic Meta data will be an **EU-market lens** on product ad activity (a good proxy for global dropshipping trends), while Egypt/US/global checks stay manual via the web UI.
...
GET https://graph.facebook.com/v23.0/ads_archive
  ...
  &limit=25
```
Control test: same query with `ad_reached_countries=['EG']` — expect no commercial results, confirming the regional scope split. (A credential-free probe from the research sandbox was blocked by its own egress proxy, so the documented 400-OAuth-error auth gate was not observed live.)
...
- <https://about.fb.com/news/2019/06/offering-greater-transparency/> — Egypt on the transparency-enforcement expansion list
...
**Country/entity dependencies:** identity confirmation availability for Egyptian ID is high-confidence but login-gated (confirm by attempting); no entity required anywhere; third-party tools constrained only by payment method availability in Egypt.
